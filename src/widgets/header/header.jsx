  ...
  return (
    <div>
      <section className="flex text-2xl max-w-[85%] m-auto justify-between my-5 items-center">
        <Link to="/">
          <h1 className="md:text-5xl font-bold text-[#9F00FF] cursor-pointer hover:transition hover:translate-0.5 hover:duration-500">EVENT.TJ</h1>
        </Link>
          ...
          <div>
            <section className="flex text-2xl max-w-[85%] m-auto justify-between my-5 items-center dark:bg-gray-800">
              <div className="relative" ref={dropdownRef}>
                <button onClick={() => setOpen(!open)} className="flex items-center gap-2 border-2 border-[#9F00FF] rounded bg-white dark:bg-gray-800 px-3 py-1 cursor-pointer">
                  <img src={currentLang.flag} alt={currentLang.label} className="w-6 h-4 object-cover" />
                  <span>{currentLang.label}</span>
                  <svg className={`w-4 h-4 ml-1 transition-transform ${open ? "rotate-180" : "rotate-0"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                {open && (
                  <ul className="absolute right-0 mt-1 bg-white border border-gray-300 rounded shadow-md z-10 w-36">
                    {LANGUAGES.map(({ code, label, flag }) => (
                      <li key={code} onClick={() => changeLanguage(code)} className="flex items-center dark:bg-gray-800 gap-2 px-3 py-2 hover:bg-[#9F00FF] hover:text-white cursor-pointer">
                        <img src={flag} alt={label} className="w-6 h-4 object-cover" />
                        {label}
                      </li>
                    ))}
                  </ul>
